      ...
      <section class="content-header">
        <div class="container-fluid">
          <div class="d-flex flex-row justify-content-between mb-3">
            <h1>Readed Message</h1>
            <div class="btn-group" role="group" aria-label="Basic example">
              <a href="message.php" type="button" class="btn btn-warning">Back</a>
              <a href="model/message_action.php?action=deleteAll" type="button" class="btn btn-danger">Delete All</a>
            </div>
          </div>
        </div><!-- /.container-fluid -->
      </section>
                ...
                <div class="card-body table-responsive p-0" style="height: 300px;">
                  <table id=" " class="table table-bordered table-head-fixed text-nowrap">
                    ...
                    <tbody>
                      ...
                      JOIN user_data ON user_data.user_id = user_message.user_id 
                      WHERE user_message.status = 'READED' AND user_data.status = 'ACTIVE' ORDER BY user_message.date_post DESC ");
                      while ($message_data = mysqli_fetch_array($result)) {
                      ?>
                        <?php
                        $no += 1;
                        ...
                        echo "<td>" . $message_data['date_post'] . "</td>";
                        echo "<td>
                        <a class='btn btn-info btn-sm btn-warning' href='model/message_action.php?id=$message_data[message_id]&action=delete'>
