...
<body class="bg-light">
    ...
    // CEK IP DAN MENAMBAHKAN VIEWS KE ARTIKEL BLOG
    $user_ip = $_SERVER['REMOTE_ADDR'];
    $check_ip = mysqli_query($conn, "SELECT user_ip FROM blog_view WHERE judul='$judul_artikel' AND user_ip='$user_ip'");
    if (mysqli_num_rows($check_ip) >= 1) {
    } else {
        $insertview = mysqli_query($conn, "INSERT INTO blog_view VALUES('','$judul_artikel','$user_ip')");
        $updateview = mysqli_query($conn, "UPDATE blog SET views = views+1 WHERE judul='$judul_artikel'");
    }
    ?>
    <div class="container">
