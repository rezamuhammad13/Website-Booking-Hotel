...
<body class="bg-light">
    ...
    $query_blog = mysqli_query($conn, "SELECT * FROM blog WHERE status = 'ACTIVE' ORDER BY date_post DESC");
    $jumlah_data = mysqli_num_rows($query_blog);
    $total_halaman = ceil($jumlah_data / $batas);

    $data_blog = mysqli_query($conn, "SELECT * FROM blog WHERE status = 'ACTIVE' ORDER BY date_post DESC LIMIT $halaman_awal, $batas");
